...
This class represents individual robots with the position, the orientation and the instruction processing logic. Each robot can turn left (left 90 degrees), turn right (right 90 degrees) and move forward. The robots are processed one at a time, to allow later robots to benefit from scents left by earlier lost robots.
